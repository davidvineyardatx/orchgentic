...
from orchgentic.connectors.gmail.client import reply_message
...
class GmailReplyTool(BaseTool):
    ...
    async def execute(self, message_id: str, body: str, confirm: bool = False, connection: str | None = None, **kwargs):
        try:
            self.policy.enforce_action(self.name, confirm)
            sent = reply_message(self._connection(connection), message_id, body)
            return ToolResult(True, self.name, data={"message_id": sent.get("id"), "thread_id": sent.get("threadId"), "status": "reply_sent"})
